...
# Warte, bis MariaDB hochgefahren ist (temporärer Client-Container)
until docker run --rm --network container:bookstack-db mariadb:11 \
    sh -c "mysql -h 127.0.0.1 -u root -p\$MYSQL_ROOT_PASSWORD -e 'SELECT 1'" 2>/dev/null; do
  sleep 2
done
